...
@interface MASGuideViewConstraint (test)
...
SpecBegin(MASGuideViewConstraint) {
    MASConstraintDelegateMock *delegate;
    MAS_VIEW *superview;
    MASGuideViewConstraint *constraint;
    MAS_VIEW *otherView;
}
...
- (void)setUp {
    ...
    constraint = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:view.mas_width];
...
- (void)testSuperClassMethod {
    expect(constraint).respondTo(@selector(installedView));
    expect(constraint).respondTo(@selector(layoutConstraint));
    expect(constraint).respondTo(@selector(layoutRelation));
    expect(constraint).respondTo(@selector(layoutPriority));
    expect(constraint).respondTo(@selector(layoutMultiplier));
    expect(constraint).respondTo(@selector(layoutConstant));
    expect(constraint).respondTo(@selector(hasLayoutRelation));
    expect(constraint).respondTo(@selector(mas_key));
    expect(constraint).respondTo(@selector(useAnimator));
    expect(constraint).respondTo(@selector(supportsActiveProperty));
    expect(constraint).respondTo(@selector(hasBeenInstalled));
}

- (void)testSupportsActiveProperty {
    expect([constraint supportsActiveProperty]).equal([constraint layoutConstraint].active);
}


- (void)testRelationCreateEqualConstraint {
    MASViewAttribute *secondViewAttribute = otherView.mas_top;
    MASGuideViewConstraint *newConstraint = (id)constraint.equalTo(secondViewAttribute);
...
- (void)testRelationCreateGreaterThanOrEqualConstraint {
    MASViewAttribute *secondViewAttribute = otherView.mas_top;
    MASGuideViewConstraint *newConstraint = (id)constraint.greaterThanOrEqualTo(secondViewAttribute);
...
- (void)testRelationCreateLessThanOrEqualConstraint {
    MASViewAttribute *secondViewAttribute = otherView.mas_top;
    MASGuideViewConstraint *newConstraint = (id)constraint.lessThanOrEqualTo(secondViewAttribute);
...
- (void)testRelationAcceptsValueWithCGPoint {
    ...
    MASGuideViewConstraint *centerX = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_centerX];
    centerX.equalTo(value);
    expect(centerX.layoutConstant).to.equal(10);
    
    MASGuideViewConstraint *centerY = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_centerY];
    centerY.equalTo(value);
    expect(centerY.layoutConstant).to.equal(20);
    
    MASGuideViewConstraint *width = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_width];
    width.equalTo(value);
...
- (void)testRelationAcceptsValueWithCGSize {
    ...
    MASGuideViewConstraint *height = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_height];
...
- (void)testRelationAcceptsValueWithEdgeInsets {
    ...
    MASGuideViewConstraint *top = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_top];
    top.equalTo(value);
    expect(top.layoutConstant).to.equal(10);
    
    MASGuideViewConstraint *left = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_left];
    left.equalTo(value);
    expect(left.layoutConstant).to.equal(20);

    MASGuideViewConstraint *leading = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_leading];
    leading.equalTo(value);
    expect(leading.layoutConstant).to.equal(20);

    MASGuideViewConstraint *bottom = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_bottom];
    bottom.equalTo(value);
    expect(bottom.layoutConstant).to.equal(-30);

    MASGuideViewConstraint *right = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_right];
    right.equalTo(value);
    expect(right.layoutConstant).to.equal(-40);

    MASGuideViewConstraint *trailing = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:otherView.mas_trailing];
...
- (void)testRelationCreatesCompositeWithArrayOfViews {
    ...
    for (MASGuideViewConstraint *childConstraint in composite.childConstraints) {
...
- (void)testInstallRelativeToSuperview {
    MAS_VIEW *view = MAS_VIEW.new;
    constraint = [[MASGuideViewConstraint alloc] initWithFirstViewAttribute:view.mas_baseline];
    constraint.delegate = delegate;
    [superview addSubview:view];
...
- (void)testAttributeChainingShouldCallDelegate {
    MASGuideViewConstraint *result = (id)constraint.and.bottom;
